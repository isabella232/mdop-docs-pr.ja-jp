...
-   **テスト環境で gpo をテストします。** テストしないで、Gpo を運用環境に展開するのは危険です。 代わりに、テストフォレストのドメインで Gpo をテストして、Gpo をファイルにエクスポートし、実稼働フォレストのドメインにインポートします。 また、テストコンピューターとユーザーを含む組織単位に Gpo をリンクすることもできます。 各 GPO がテスト環境で正しく動作することを確認してから、その Gpo を運用環境に展開します。
